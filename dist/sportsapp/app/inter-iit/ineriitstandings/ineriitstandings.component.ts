...
let fourth = [0, 0, 0, 0, 0, 0, 0, 0];
let pts = [0, 0, 0, 0, 0, 0, 0, 0];


@Component({
  selector: 'app-ineriitstandings',
  templateUrl: './ineriitstandings.component.html',
  styleUrls: ['./ineriitstandings.component.css']
})
export class IneriitstandingsComponent implements OnInit {
  InteriitPts: any =[];
  College: any = [];
  college: any = [];
  ...
  ngOnInit() {
    ...
    this.Pts.forEach(element => {
      ...
      for (let index = 0; index < this.Pts.length; index++) {
        ...
        for (let i = 0; i < 8; i++) {
          for (let n = 0; n < this.College.length; n++) {
            if (this.Pts[index].Hostel[i] == this.College[n]) {
    ...
    this.headers = ["College", "Gold", "Silver", "Bronze", "Pts"];
